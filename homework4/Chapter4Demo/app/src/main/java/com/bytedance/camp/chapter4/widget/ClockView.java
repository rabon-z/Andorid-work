...
public class ClockView extends View {
    ...
    private void init() {
        ...
        needlePaint.setColor(Color.WHITE);//为画笔初始化颜色
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float MinuteDegree;
        float SecondDegree;
        SecondDegree = (float)second / 60 * 360-90;
        MinuteDegree = (float)minute / 60 * 360+ second/10-90;
        HourDegree   = (float)hour / 12 * 360+(minute /2)-90;
        float endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(HourDegree)));
        ...
        float endX1 = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(MinuteDegree)));
        float endY1 = (float) (centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(MinuteDegree)));
        canvas.drawLine(centerX,centerY,endX1,endY1,needlePaint);
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        float endX2 = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO* Math.cos(Math.toRadians(SecondDegree)));
        float endY2 = (float) (centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(SecondDegree)));
        canvas.drawLine(centerX,centerY,endX2,endY2,needlePaint);
    }
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        float a, b;
        for (int i = 1; i<=12; i++) {
            double rad = 2 * Math.PI / 12 * (i-3);
            a = centerX -35+ (float) (Math.cos(rad) * (radius/1.2));
            b = centerY +20+ (float) (Math.sin(rad) * (radius/1.2));//为何centerX与centerY不是正中心的点？
            if(i<=9)
               canvas.drawText("0"+i+"" , a, b , numberPaint);
            else
                canvas.drawText(i+"" , a, b , numberPaint);
        }
